...
public class bl_KeyOptionsUI : MonoBehaviour
{
    ...
    [Header("References")]
    [SerializeField]private GameObject KeyOptionPrefab = null;
    [SerializeField]private Transform KeyOptionPanel = null;
    [SerializeField]private GameObject WaitKeyWindowUI = null;
    [SerializeField]private Text WaitKeyText = null;
#if TMP
    [SerializeField] private TextMeshProUGUI WaitKeyTextTMP = null;
#endif
...
#if TMP
                    if (WaitKeyTextTMP != null)
                        WaitKeyTextTMP.text = string.Format("KEY <b>'{0}'</b> IS ALREADY USE, \n PLEASE PRESS ANOTHER KEY FOR REPLACE <b>{1}</b>", vKey.ToString().ToUpper(), WaitFunctionKey.Description.ToUpper());
#endif
                }
                else
                {
    ...
    public void SetWaitKeyProcess(bl_KeyInfo info)
    {
        ...
        WaitForKey = true;
        if (WaitKeyText != null) WaitKeyText.text = string.Format("PRESS A KEY FOR REPLACE <b>{0}</b>", info.Description.ToUpper());
#if TMP
        if (WaitKeyTextTMP != null)
            WaitKeyTextTMP.text = string.Format("PRESS A KEY FOR REPLACE <b>{0}</b>", info.Description.ToUpper());
#endif
